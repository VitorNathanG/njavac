// Regression: dead + redundant `goto` compaction observed in pinned output.
// A nested constant-operand short-circuit like `(!(A || false)) || false` makes
// njavac's eager branch emitter leave behind two `goto`s absent from pinned output: one
...
// byte. Fuzzer-found (Fuzz0000073). Pinned output keeps one append[int,int] merge frame.
