use crate::Config;
use crate::javac::{JavacWorker, derive_java, worker_src_path};
use crate::observe::{ObserveWorker, Termination, observer_src_path};
...
pub(crate) fn verify_observer(cfg: &Config) -> i32 {
    ...
    let reference = compile_probe(&mut javac, NAME, "System.out.println(\"reference\");");
    let candidate = compile_probe(&mut javac, NAME, "System.out.println(\"candidate\");");
    let same = observer.observe_pair(NAME, &reference, &reference);
    assert_eq!(
        same.reference, same.candidate,
        "observer disagreed on identical classes"
    );
    ...
    assert_ne!(
        different.reference, different.candidate,
        "observer missed stdout divergence"
    );
    ...
    assert_eq!(
        candidate_timeout.reference.termination,
        Termination::Returned
    );
    assert_eq!(
        candidate_timeout.candidate.termination,
        Termination::TimedOut
    );

    let restarted = observer.observe_pair(NAME, &reference, &reference);
    assert_eq!(
        restarted.reference, restarted.candidate,
        "observer failed after timeout restart"
    );
...
fn compile_probe(worker: &mut JavacWorker, name: &str, body: &str) -> Vec<u8> {
    let source =
        format!("public class {name} {{ public static void main(String[] args) {{ {body} }} }}");
    let classes = worker.compile_batch(&[(name, &source)]);
    assert_eq!(
        classes.len(),
        1,
        "observer probe javac emitted an unexpected class set"
    );
    classes
        .get(name)
        .cloned()
