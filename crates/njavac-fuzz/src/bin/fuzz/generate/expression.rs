...
const SHORT_LIT: [i32; 12] = [
    -32768, -129, -128, -1, 0, 1, 127, 128, 255, 256, 32766, 32767,
];
...
impl Gen {
        ...
        *budget -= 1;
        ...
        match target {
            Byte | Short | Char => {
                ...
                FExpr::Cast(
                    target,
                    Box::new(self.expr(env, src, BoolMode::Value, budget)),
                )
            }
    ...
    fn numeric_compound(&mut self, env: &[Ty], target: Ty, budget: &mut i32) -> FExpr {
        ...
        match &forms[idx] {
            ...
            Form::BitNot => {
                FExpr::BitNot(Box::new(self.expr(env, target, BoolMode::Value, budget)))
            }
            Form::Cast => {
                let src = *self.rng.pick(&NUMERIC);
                FExpr::Cast(
    ...
    fn bool_expr(&mut self, env: &[Ty], mode: BoolMode, budget: &mut i32) -> FExpr {
        ...
        match self.rng.below(if CAPS.boolean_boundaries { 8 } else { 6 }) {
            ...
            5 if CAPS.boolean_boundaries => FExpr::Cast(
                Boolean,
                Box::new(self.bool_expr(env, BoolMode::Branch, budget)),
            ),
    ...
    fn leaf(&mut self, env: &[Ty], target: Ty) -> FExpr {
        ...
        match target {
            ...
            Byte => FExpr::Cast(
                Byte,
                Box::new(FExpr::Lit(Val::I(*self.rng.pick(&BYTE_LIT)))),
            ),
            Short => FExpr::Cast(
                Short,
                Box::new(FExpr::Lit(Val::I(*self.rng.pick(&SHORT_LIT)))),
            ),
        }
    }
